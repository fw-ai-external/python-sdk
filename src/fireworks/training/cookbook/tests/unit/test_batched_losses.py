...
from fireworks.training.cookbook.utils.rl.losses import make_grpo_loss_fn, _normalize_prompt_lens
...
class TestGRPOLossBatched:
    def test_single_int_equals_list(self):
        ...
        fn_int = make_grpo_loss_fn(adv, ref, prompt_len=10, kl_beta=0.01)
        fn_list = make_grpo_loss_fn(adv, ref, prompt_len=[10, 10], kl_beta=0.01)
    ...
    def test_mixed_prompt_lens(self):
        ...
        fn_short = make_grpo_loss_fn(adv[:1], ref[:1], prompt_len=2, kl_beta=0.0)
        fn_long = make_grpo_loss_fn(adv[:1], ref[:1], prompt_len=8, kl_beta=0.0)
    ...
    def test_gradient_equivalence(self):
        ...
        fn_a = make_grpo_loss_fn(adv_a, ref_a, prompt_len=prompt_len_a, kl_beta=0.01)
        fn_b = make_grpo_loss_fn(adv_b, ref_b, prompt_len=prompt_len_b, kl_beta=0.01)
        ...
        fn_combined = make_grpo_loss_fn(combined_adv, combined_ref, prompt_len=combined_lens, kl_beta=0.01)
